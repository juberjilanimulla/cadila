...
viewpdfRouter.get("/:id", async (req, res) => {
  try {
    ...
    if (!id) {
      return errorResponse(res, 400, "Missing applicant ID");
    }

    const applicant = await jobapplicantsmodel.findById(id);

    if (!applicant || !applicant.resume) {
      return errorResponse(res, 404, "Resume not found");
    }

    // Resume is now a Google Drive link
    return successResponse(res, "Resume link fetched", {
